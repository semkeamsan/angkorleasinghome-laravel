                ...
                <div class="product-item__header">
                    <div class="position-relative">
                        <a href="{{ $row->getDetailUrl() }}" class="d-block">
                            <img class="min-height-230 bg-img-hero card-img-top" src="{{ $row->image_url }}"
            ...
            <div class="col-md-7 col-xl-5 col-wd-4gdot5 flex-horizontal-center">
                <div class="w-100 position-relative m-4 m-md-0">
                    ...
                    <a @if (!empty($blank)) target="_blank" @endif href="{{ $row->getDetailUrl() }}">
                        <span class="font-weight-medium font-size-17 text-dark d-flex mb-1">{{ $translation->title }}
                        </span>
                    </a>
                    <div class="card-body p-0">
                        <div class="d-flex flex-wrap flex-xl-nowrap align-items-center font-size-14 text-gray-1">
                            @if (!empty($row->location->name))
                                ...
                                <i
                                    class="icon flaticon-placeholder mr-2 font-size-20"></i>{{ $location->name ?? '' }}
                        ...
                        @endphp
                        <span class="attr-title mb-1 d-block"><i class="icofont-medal"></i>
                            {{ $translate_attribute->name ?? '' }}: </span>
                        <ul class="list-unstyled mb-2 d-flex flex-lg-wrap flex-xl-wrap">
                            @foreach ($termsByAttribute as $key => $term)
                                @if ($key < 3)
                                    ...
                                    <li
                                        class="item {{ $term->slug }} term-{{ $term->id }} border border-dark rounded-xs d-flex align-items-center text-lh-1 py-1 px-2 mr-2 mb-2 mb-md-0 mb-lg-2 mb-xl-2">
                                        <span
                                            class="font-weight-normal font-size-14">{{ $translate_term->name }}</span>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
                        ...
                        @if (!empty($translation->badge_tags))
                            <ul class="list-unstyled d-flex pb-2 flex-wrap">
                                @foreach ($translation->badge_tags as $key => $item)
                                    <li
                                        class="border border-{{ $item['color'] }} bg-{{ $item['color'] }} rounded-xs d-flex align-items-center text-lh-1 py-1 px-3 mb-2 mr-2">
