...
<div class="card transition-3d-hover shadow-hover-2 mt-2 item-loop {{$wrap_class ?? ''}}">
    <div class="position-relative">
        <a href="{{$row->getDetailUrl(false)}}" class="d-block gradient-overlay-half-bg-gradient-v5">
            @if($row->image_url)
    ...
    <div class="card-body px-4 pt-2 pb-3">
        ...
        @endif
        <a href="{{$row->getDetailUrl(false)}}" class="card-title font-size-17 font-weight-medium text-dark">{{$translation->title}}</a>
