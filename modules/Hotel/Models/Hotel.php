...
class Hotel extends Bookable
{
    ...
    public function termsByAttributeInListingPage()
    {
        ...
        return $this->hasManyThrough($this->termClass, $this->hotelTermClass, 'target_id', 'id', 'id', 'term_id')->where('bc_terms.attr_id', $attribute)->with(['translations']);
    }
    ...
    $raw_sql_min_max .= "Or (  bc_hotels.price_year >=  $pri_from )
        ...
        if (is_array($terms) && !empty($terms)) {
            ...
            if(!empty($terms)){
                $model_hotel->join('bc_hotel_term as tt', 'tt.target_id', "bc_hotels.id")->whereIn('tt.term_id', $terms);
            }
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_hotels.review_score >= ? AND bc_hotels.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_hotel->leftJoin('bc_hotel_translations', function ($join) {
                    $join->on('bc_hotels.id', '=', 'bc_hotel_translations.origin_id');
                });
                $model_hotel->where('bc_hotel_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_hotel->where('bc_hotels.title', 'LIKE', '%' . $service_name . '%');
            }
        }
