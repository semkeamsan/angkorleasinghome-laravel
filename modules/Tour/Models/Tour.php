...
class Tour extends Bookable
{
    ...
    {
        return $this->belongsTo(TourCategory::class,'category_id')->with('translations')->withDefault();
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            $number = parent::join('bc_locations', function ($join) use ($location) {
                $join->on('bc_locations.id', '=', 'bc_tours.location_id')->where('bc_locations._lft', '>=', $location->_lft)->where('bc_locations._rgt', '<=', $location->_rgt);
            })->where("bc_tours.status", "publish")->with(['translations'])->count("bc_tours.id");
        }
        if (empty($number))
            return false;
        if ($number > 1) {
    ...
    {
        $model_Tour = parent::query()->select("bc_tours.*");
        ...
        if (!empty($category_ids = $request->query('cat_id'))) {
            ...
            if (!empty($list_cat)) {
                ...
                foreach ($list_cat as $cat) {
                    $where_left_right[] = " ( bc_tour_category._lft >= ? AND bc_tour_category._rgt <= ? ) ";
                    $params[] = $cat->_lft;
                    $params[] = $cat->_rgt;
                }
                $sql_where_join = " ( " . implode("OR", $where_left_right) . " )  ";
                $model_Tour->join('bc_tour_category', function ($join) use ($sql_where_join,$params) {
                        $join->on('bc_tour_category.id', '=', 'bc_tours.category_id')->WhereRaw($sql_where_join,$params);
                    });
            }
        }
        ...
        if (is_array($terms) && !empty($terms)) {
            ...
            if(!empty($terms)){
                $model_Tour->join('bc_tour_term as tt', 'tt.term_id', "bc_tours.id")->whereIn('tt.term_id', $terms);
            }
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number) {
                $where_review_score[] = " ( bc_tours.review_score >= ? AND bc_tours.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if (!empty($service_name = $request->query("service_name"))) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $model_Tour->leftJoin('bc_tour_translations', function ($join) {
                    $join->on('bc_tours.id', '=', 'bc_tour_translations.origin_id');
                });
                $model_Tour->where('bc_tour_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $model_Tour->where('bc_tours.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_Tour->orderByRaw("POW((bc_tours.map_lng-?),2) + POW((bc_tours.map_lat-?),2)",[$lgn,$lat]);
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_tours.sale_price, 0 ) > 0 THEN bc_tours.sale_price ELSE bc_tours.price END AS tmp_min_price";
